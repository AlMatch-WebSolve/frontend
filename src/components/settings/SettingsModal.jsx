...
const DEFAULT_SHORTCUTS = [
  ...
  { key: "Ctrl+Z", ko: "문자삽입취소", en: "Undo Insert Characters", desc: "문자 삽입 취소" },
  { key: "Ctrl+C", ko: "복사", en: "Copy", desc: "복사" },
  { key: "Ctrl+V", ko: "붙여넣기", en: "Paste", desc: "붙여넣기" },
  { key: "Ctrl+A", ko: "모두선택", en: "Select All", desc: "모두 선택" },
  { key: "Ctrl+/", ko: "주석 전환", en: "Toggle Comment", desc: "주석 생성 및 삭제" },
  { key: "Ctrl+'+'/ Ctrl+'-'", ko: "확대/축소", en: "ZoomIn / ZoomOut", desc: "글자 크기 확대 / 축소" },
];
...
export default function SettingsModalLocal({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 테마 매핑 (서버 → UI)
        const rawTheme = String(data?.theme ?? initialTheme).trim().toUpperCase();
  ...
  const onConfirm =
    confirmKind === 'save' ? handleConfirmSave
      : confirmKind === 'logout' ? handleConfirmLogout
        : undefined;
    ...
    <div className={styles.backdrop} onClick={saving ? undefined : onClose}>
      ...
      >
        <div className={styles.settingsHeader}>
          <p id="settings-title" className={styles.settingsTitle}>설정</p>
          <button
            aria-label="닫기"
        ...
        <div className={styles.settingsInner} aria-busy={loading || saving}>
          ...
          <div className={styles.tableWrap}>
            <table className={styles.table} aria-label="단축키 목록">
              <thead>
                <tr>
                  <th>단축키</th><th>한글</th><th>영어</th><th>설명</th>
                </tr>
              </thead>
              <tbody>
                {shortcuts.map((s) => (
          ...
          <div className={styles.optionsRow}>
            ...
            <div className={styles.optionGroup}>
              <label className={styles.optionLabel} htmlFor="langSelect">언어 선택</label>
              <select
                id="langSelect"
